...
// Skeleton adjusted for light theme background
const PublicProfilePageSkeleton = () => (
  <div className="min-h-screen bg-gray-50">
    <div className="container mx-auto max-w-7xl space-y-6 p-4 md:p-6 pt-24">
      <Skeleton className="h-48 w-full rounded-3xl bg-gray-200" /> {/* Header */}
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="col-span-1 space-y-6"> {/* Left Column */}
          <Skeleton className="h-64 w-full rounded-2xl bg-gray-200" /> {/* Profile Info */}
          <Skeleton className="h-40 w-full rounded-2xl bg-gray-200" /> {/* Business Details */}
        </div>
        <div className="col-span-1 space-y-6 md:col-span-2"> {/* Right Column */}
          <Skeleton className="h-40 w-full rounded-2xl bg-gray-200" /> {/* About */}
          <Skeleton className="h-40 w-full rounded-2xl bg-gray-200" /> {/* Offerings */}
          <Skeleton className="h-40 w-full rounded-2xl bg-gray-200" /> {/* Interests */}
        </div>
      </div>
    </div>
  </div>
        ...
        <main className="px-4 py-8 max-w-7xl mx-auto">
          <div className="mb-8">
            <div className="space-y-6">
              ...
              {/* Profile Header Wrapper - Light Theme */}
              <div className="bg-white border border-gray-200 shadow-sm mx-auto rounded-3xl overflow-hidden">
                <ProfileHeader profile={profile} />
              </div>
              
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 space-y-6">
                  {/* ProfileInfo Wrapper - Light Theme */}
                  <div className="bg-white border border-gray-200 p-6 shadow-sm rounded-2xl h-full">
                    <ProfileInfo profile={profile} />
                  </div>
                </div>
                
                <div className="lg:col-span-2 space-y-6">
                  {/* AboutCompany contains Description, Vision, and Mission */}
                  <AboutCompany profile={profile} /> 
                  
                  {/* Business & Legal Details combined */}
                  <BusinessDetails profile={profile} />
                </div>
              </div>
            </div>
