...
import { X, Briefcase, Users, FlaskConical, Heart, Sparkles, ArrowRight, Upload, Image as ImageIcon } from "lucide-react"
...
const TABS_CONFIG = [
    { id: "live-projects", label: "Live Projects", icon: Briefcase, color: "from-violet-500 to-purple-600" },
    { id: "internship", label: "Internship", icon: Users, color: "from-blue-500 to-cyan-600" },
    { id: "research", label: "Research", icon: FlaskConical, color: "from-emerald-500 to-teal-600" },
    { id: "csr-initiative", label: "CSR Initiative", icon: Heart, color: "from-rose-500 to-pink-600" },
]
...
const FormField = ({ label, id, children, isTextarea = false, required = false }) => (
    <div className="relative group">
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50 transition-all duration-300 hover:border-purple-500/50 relative z-10">
            <Label htmlFor={id} className="block text-sm font-semibold text-gray-200 mb-3 flex items-center gap-2">
                {label}
...
const StyledInput = ({ id, placeholder, icon: Icon, value, onChange, ...props }) => (
    <div className="relative">
        {Icon && <Icon className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 z-20" />}
        <Input
            id={id}
            ...
            className={`h-14 text-base rounded-xl border-2 border-gray-600/50 focus:border-purple-500 focus:ring-0 transition-all duration-300 bg-gray-700/80 backdrop-blur-sm text-white placeholder-gray-400 relative z-10 ${Icon ? "pl-12" : "pl-4"
                }`}
            {...props}
        />
    </div>
...
const CheckboxGroup = ({ options, name, type = "checkbox", selectedValue, onChange, formData }) => {
    return (
                ...
                {options.map((option) => (
                    <div key={option.id} className="group">
                        <div
                            ...
                            onClick={() => type === "radio" && onChange({ target: { name, value: option.id } })}
                        >
                            {type === "radio" ? (
                                <input
                                    type="radio"
                                ...
                                <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                        if (checked) {
                                            onChange({ target: { name, value: [...(selectedValue || []), option.id] } })
                                        } else {
                                            onChange({ target: { name, value: (selectedValue || []).filter(item => item !== option.id) } })
                                        }
                                    }}
                                    className="border-2 border-gray-500 w-5 h-5 data-[state=checked]:bg-purple-500 data-[state=checked]:border-purple-500"
                                />
                            )}
                            <Label htmlFor={option.id} className="text-sm font-medium text-gray-200 cursor-pointer flex-1">
                                {option.label}
                            </Label>
                        </div>
                        {/* Conditional rendering for "Please specify" field for compensation/stipend */}
                        {(option.label.includes("Please specify") && selectedValue === option.id) && (
                            <div className="mt-2 ml-8">
                                <StyledInput
                                    ...
                                    className={`h-10 text-sm ${selectedValue === option.id ? 'border-red-500 focus:border-red-500' : ''}`}
                                    icon={undefined}
                                ...
                                {selectedValue === option.id && (
                                    <p className="text-red-400 text-xs mt-1">* Required when this option is selected</p>
                                )}
                            </div>
                        )}
                    </div>
...
const SectionHeader = ({ icon: Icon, title, description, gradientFrom, gradientTo }) => (
...
const ImageUpload = ({ label, id, required = false, onImageChange, previewUrl, setPreviewUrl }: { label: string; id: string; required?: boolean; onImageChange?: (data: any) => void; previewUrl: string | null; setPreviewUrl: (url: string | null) => void }) => {
...
const LiveProjectsForm = ({ onSubmit, formData, handleChange, onImageUpload, previewUrl, setPreviewUrl }) => {
    const modeOptions = [
        ...
        <div className="px-6 sm:px-12 py-12">
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                <FormField label="Project Title" id="projectTitle" required>
                    <StyledInput id="projectTitle" placeholder="e.g., AI-Powered Customer Analytics Dashboard" icon={undefined} value={formData.projectTitle || ""} onChange={handleChange} />
                </FormField>

                <FormField label="Project Description" id="projectDescription" isTextarea required>
                    <StyledTextarea
                ...
                <ImageUpload label="Project Image/Poster" id="projectImage" onImageChange={onImageUpload} previewUrl={previewUrl} setPreviewUrl={setPreviewUrl} />

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Skills Required" id="projectSkills" required>
                        <StyledInput id="projectSkills" placeholder="e.g., React, Node.js, Python, UI/UX Design" icon={undefined} value={formData.projectSkills || ""} onChange={handleChange} />
                    </FormField>
                    <FormField label="Duration" id="projectDuration" required>
                        <StyledInput id="projectDuration" placeholder="e.g., 2 months, 6 months, 1 year" icon={undefined} value={formData.projectDuration || ""} onChange={handleChange} />
                    </FormField>
                </div>

                <FormField label="Mode of Work" id="projectMode">
                    <CheckboxGroup options={modeOptions} name="projectMode" type="radio" selectedValue={formData.projectMode} onChange={handleChange} formData={formData} />
                </FormField>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Team Size" id="projectTeamSize">
                        <StyledInput id="projectTeamSize" placeholder="e.g., 3-5 developers, 10+ team members" icon={undefined} value={formData.projectTeamSize || ""} onChange={handleChange} />
                    </FormField>
                    {/* Location field as a simple input. Backend will use projectModeLocation if available, else projectLocation */}
                    <FormField label="Location (e.g., Mumbai, Maharashtra, Remote/Online)" id="projectLocation">
                        <StyledInput id="projectLocation" placeholder="e.g., Mumbai, Maharashtra, Remote/Online" icon={undefined} value={formData.projectLocation || ""} onChange={handleChange} />
                    </FormField>
                </div>

                <FormField label="Compensation" id="projectCompensation">
                    <CheckboxGroup options={compensationOptions} name="projectCompensation" type="radio" selectedValue={formData.projectCompensation} onChange={handleChange} formData={formData} />
                </FormField>

                <FormField label="Project Extendable" id="projectExtendable">
                    <CheckboxGroup options={extendableOptions} name="projectExtendable" type="radio" selectedValue={formData.projectExtendable} onChange={handleChange} formData={formData} />
                </FormField>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Email" id="projectEmail" required>
                        <StyledInput id="projectEmail" placeholder="e.g., project@company.com" type="email" icon={undefined} value={formData.projectEmail || ""} onChange={handleChange} />
                    </FormField>
                    <FormField label="Phone Number" id="projectPhone" required>
                        <StyledInput id="projectPhone" placeholder="e.g., +91-9876543210" type="tel" icon={undefined} value={formData.projectPhone || ""} onChange={handleChange} />
                    </FormField>
                </div>

                <FormField label="Send your CV at" id="projectCvEmail" required>
                    <StyledInput id="projectCvEmail" placeholder="e.g., careers@company.com, hr@company.com" type="email" icon={undefined} value={formData.projectCvEmail || ""} onChange={handleChange} />
                </FormField>
...
const InternshipForm = ({ onSubmit, formData, handleChange, onImageUpload, previewUrl, setPreviewUrl }) => {
  const stipendOptions = [
      ...
      { id: "paid-intern-above-6k", label: "Above ₹6,000/month - Please specify" },
  ]
      ...
      <div className="px-6 sm:px-12 py-12">
          ...
          <div className="max-w-5xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <FormField label="Email" id="contact_email" required> {/* Renamed for clarity */}
                      <StyledInput
                          id="contact_email"
                  ...
                  <FormField label="Phone Number" id="contact_phone" required> {/* Renamed for clarity */}
                      <StyledInput
                          id="contact_phone"
              ...
              <FormField label="Send your CV at" id="internship_cv_email" required> {/* Renamed for clarity */}
                  <StyledInput
...
const ResearchForm = ({ onSubmit, formData, handleChange, onImageUpload, previewUrl, setPreviewUrl }) => {
    const stipendOptions = [
        ...
        { id: "paid-research-above-6k", label: "Above ₹6,000/month - Please specify" },
    ]
        ...
        <div className="px-6 sm:px-12 py-12">
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                <FormField label="Research Title" id="researchTitle" required>
                    <StyledInput id="researchTitle" placeholder="e.g., AI in Healthcare: Predictive Analytics Study" icon={undefined} value={formData.researchTitle || ""} onChange={handleChange} />
                </FormField>

                <FormField label="Research Description" id="researchDescription" isTextarea required>
                    <StyledTextarea
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Open For" id="researchOpenFor">
                        <StyledInput id="researchOpenFor" placeholder="e.g., Masters students, PhD candidates, Postdocs" icon={undefined} value={formData.researchOpenFor || ""} onChange={handleChange} />
                    </FormField>
                    <FormField label="Duration" id="researchDuration" required>
                        <StyledInput id="researchDuration" placeholder="e.g., 6 months, 1 year, 2-3 years (PhD)" icon={undefined} value={formData.researchDuration || ""} onChange={handleChange} />
                    </FormField>
                </div>

                <FormField label="Stipend" id="researchStipend">
                    <CheckboxGroup options={stipendOptions} name="researchStipend" type="radio" selectedValue={formData.researchStipend} onChange={handleChange} formData={formData} />
                </FormField>

                <FormField label="Skills Required" id="researchSkills">
                    <StyledInput id="researchSkills" placeholder="e.g., Python/R, Statistical analysis, Literature review" icon={undefined} value={formData.researchSkills || ""} onChange={handleChange} />
                </FormField>

                <FormField label="Extendable" id="researchExtendable">
                    <CheckboxGroup options={extendableOptions} name="researchExtendable" type="radio" selectedValue={formData.researchExtendable} onChange={handleChange} formData={formData} />
                </FormField>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Email" id="researchEmail" required>
                        <StyledInput id="researchEmail" placeholder="e.g., prof.smith@university.edu" type="email" icon={undefined} value={formData.researchEmail || ""} onChange={handleChange} />
                    </FormField>
                    <FormField label="Phone Number" id="researchPhone" required>
                        <StyledInput id="researchPhone" placeholder="e.g., +91-9876543210" type="tel" icon={undefined} value={formData.researchPhone || ""} onChange={handleChange} />
                    </FormField>
                </div>
                ...
                <FormField label="Send your CV at" id="researchCvEmail" required>
                    <StyledInput
                        ...
                        type="email" icon={undefined} value={formData.researchCvEmail || ""} onChange={handleChange} />
                </FormField>
...
const CSRForm = ({ onSubmit, formData, handleChange, onImageUpload, previewUrl, setPreviewUrl }) => {
    const modeOptions = [
        ...
        <div className="px-6 sm:px-12 py-12">
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                <FormField label="Initiative Type" id="initiativeType" required>
                    <StyledInput
                        id="initiativeType"
                        placeholder="e.g., Education Support, Environmental Conservation, Healthcare Access" icon={undefined} value={formData.initiativeType || ""} onChange={handleChange} />
                </FormField>

                <FormField label="Initiative Description" id="csrDescription" isTextarea required>
                    <StyledTextarea
                        id="csrDescription"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Duration" id="csrDuration" required>
                        <StyledInput id="csrDuration" placeholder="e.g., 3 months, 6 months, Ongoing program" icon={undefined} value={formData.csrDuration || ""} onChange={handleChange} />
                    </FormField>
                    <FormField label="Team Size" id="members">
                        <StyledInput id="members" placeholder="e.g., 5-10 volunteers, 20+ team members" icon={undefined} value={formData.members || ""} onChange={handleChange} />
                    </FormField>
                </div>

                <FormField label="Mode of Work" id="mode">
                    <CheckboxGroup options={modeOptions} name="mode" type="radio" selectedValue={formData.mode} onChange={handleChange} formData={formData} />
                </FormField>

                <FormField label="Stipend" id="csrStipend">
                    <CheckboxGroup options={stipendOptions} name="csrStipend" type="radio" selectedValue={formData.csrStipend} onChange={handleChange} formData={formData} />
                </FormField>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Compensation (Free Text)" id="csrCompensation">
                        <StyledInput id="csrCompensation" placeholder="e.g., Travel reimbursement, Meals provided, Certificate" icon={undefined} value={formData.csrCompensation || ""} onChange={handleChange} />
                    </FormField>
                    ...
                    <FormField label="Location (e.g., City, State, Remote)" id="location">
                        <StyledInput id="location" placeholder="e.g., Mumbai, Maharashtra, Remote/Online" icon={undefined} value={formData.location || ""} onChange={handleChange} />
                    </FormField>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Email" id="csrEmail" required>
                        <StyledInput id="csrEmail" placeholder="e.g., csr@company.com" type="email" icon={undefined} value={formData.csrEmail || ""} onChange={handleChange} />
                    </FormField>
                    <FormField label="Phone Number" id="csrPhone" required>
                        <StyledInput id="csrPhone" placeholder="e.g., +91-9876543210" type="tel" icon={undefined} value={formData.csrPhone || ""} onChange={handleChange} />
                    </FormField>
                </div>
            ...
            <div className="relative z-10 flex items-center justify-between px-8 py-8">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-purple-200 text-lg">Post opportunities and connect with talent</p>
                    </div>
                </div>
                <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 rounded-xl h-12 w-12">
                    <X className="h-6 w-6" />
                </Button>
            </div>
                ...
                {TABS_CONFIG.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex-1 px-6 py-5 text-sm font-medium transition-all duration-300 flex items-center justify-center space-x-2 relative overflow-hidden group ${activeTab === tab.id
...
export default function EnhancedShareNeedsForm() {
    ...
    const [formData, setFormData] = useState<Record<string, any>>({}) // State to hold all form data
    const [uploadedImages, setUploadedImages] = useState<Record<string, string | null>>({}) // Stores URLs for image previews
    ...
    const handleImageUpload = (imageData: { file: File | null, url: string | null, id: string }) => {
        setUploadedImages(prev => ({
            ...prev,
            [imageData.id]: imageData.url
        }));

        // Backend expects keys like 'LiveProjectsImage', 'InternshipImage', etc. in 'details_json'
        // This is derived from `formData[`${formType.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('')}Image`]`
        const formTypePrefix = activeTab.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('');
        const backendImageKey = `${formTypePrefix}Image`;

        setFormData(prev => ({
            ...prev,
            [backendImageKey]: imageData.url // Store the URL with the correct backend-expected key
        }));
    };
    ...
    const handleSubmit = async (formType: string) => {
        ...
        // No special location handling needed here because:
        // 1. The frontend's location inputs are now simple text fields.
        // 2. The backend explicitly handles the logic:
        //    'live-projects': `location = formData.projectModeLocation || formData.projectLocation || null;`
        //    'csr-initiative': `location = formData.modeLocation || formData.location || null;`
        // So, if 'projectModeLocation' or 'modeLocation' are populated from the radio buttons,
        // the backend will pick them up. Otherwise, it will use 'projectLocation' or 'location'.
        // The frontend just needs to send what it collects.
        ...
        try {
            ...
            if (response.ok) {
                ...
                const activityPost = createActivityPost(formType, finalFormData);
                const existingPosts = JSON.parse(localStorage.getItem('activityPosts') || '[]');
                existingPosts.unshift(activityPost);
                localStorage.setItem('activityPosts', JSON.stringify(existingPosts));
                ...
                alert(`Failed to create post: ${errorData.message || response.statusText}. Please try again.`);
            }
        } catch (error) {
    ...
    const renderActiveForm = () => {
        const commonProps = {
            ...
            setPreviewUrl: (url: string | null) => handleImageUpload({ file: null, url: url, id: `${activeTab}Image` })
        }
