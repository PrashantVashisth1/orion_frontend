...
export default function FeedSidebar({ isOpen, onClose }: FeedSidebarProps) {
  if (!isOpen) return null

  const filters = [
    { name: "All Posts", count: 248, active: true },
    { name: "Following", count: 42, active: false },
    { name: "Liked Posts", count: 18, active: false },
    { name: "My Posts", count: 12, active: false },
  ]
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className="fixed right-0 top-0 h-full w-80 bg-gradient-to-br from-gray-800/95 to-gray-900/95 backdrop-blur-sm border-l border-gray-700/50 z-50 transform transition-transform duration-300">
        <div className="p-6">
          {/* Header */}
          ...
          {/* Post Filters */}
          <div className="mb-8">
            <h3 className="text-lg font-semibold text-white mb-4">Post Types</h3>
            <div className="space-y-3">
              {filters.map((filter) => (
                <button
                  key={filter.name}
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {categories.map((category) => (
                <button
                  key={category.name}
                  className="w-full flex items-center space-x-3 p-3 rounded-lg bg-gray-700/30 hover:bg-gray-600/30 transition-colors text-left"
                >
