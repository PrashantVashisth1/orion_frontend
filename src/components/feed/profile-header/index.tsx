...
export default function ProfileHeader({ profile }) {
  ...
  // const bgURL = profile?.data?.personalInfo?.profilePicture || null;
  const profilePictureUrl = profileImage || profile?.data?.companyDetails?.companyLogo || null;
  // Using a light-friendly background image
  const bgURL = "https://images.presentationgo.com/2025/04/blue-technology-wave-background.jpg";


  return (
    // Light theme: White/Light Gray background, Light border, soft shadow.
    // NOTE: We keep the background image logic but use a brighter overlay for contrast.
    <div
      className="bg-white/70 backdrop-blur-md rounded-2xl border border-gray-200 p-8 shadow-xl relative w-full overflow-hidden"
      style={{
        // Using a bright overlay over the background image
        backgroundImage: `linear-gradient(rgba(255, 255, 255, 0.8), rgba(249, 250, 251, 0.9)), url(${bgURL})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundRepeat: "no-repeat",
      }}
    >
      ...
      {/* <Button
        variant="ghost"
        size="icon"
        className="absolute top-6 right-6 text-gray-500 hover:text-gray-800 hover:bg-gray-100 z-20 rounded-full w-10 h-10"
        onClick={handleEditImage}
      >
        <Camera className="h-5 w-5" />
      </Button> */}
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center space-y-6 md:space-y-0 md:space-x-8 pr-16">
        {/* Profile Picture / Logo */}
        <div className="w-28 h-28 bg-gray-100 p-1 rounded-full flex items-center justify-center flex-shrink-0 relative overflow-hidden ring-4 ring-blue-500/50">
          {profilePictureUrl ? (
            <img
              src={profilePictureUrl}
              alt="Profile"
            ...
            // Placeholder text color adjusted for light background
            <div className="w-24 h-24 bg-gray-300 rounded-full flex items-center justify-center text-xl font-bold text-gray-700">
              {companyName.charAt(0)}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-4 mb-3">
            {/* Heading text is now dark for contrast */}
            <h1 className="text-4xl font-extrabold text-gray-900 break-words max-w-full">
              {companyName}
            </h1>
          </div>
          {/* Subtext adjusted to a dark gray */}
          <p className="text-gray-600 mb-4 text-lg font-medium">
            Founded: **{foundedYear}** {profile?.company_details?.company_location && ` · Location: ${profile.company_details.company_location}`}
          </p>

          <div className="flex flex-wrap items-center gap-3 mb-4">
            {/* Tag/Pill colors adjusted for light theme */}
            <div className="flex items-center gap-2 bg-blue-100 px-3 py-1 rounded-full border border-blue-200">
              ...
              <span className="text-sm font-bold text-blue-700">1.2K</span>
            </div>
          ...
          {/* <Button
            variant="outline"
            size="sm"
            className="bg-blue-50 border-blue-300 text-blue-600 hover:bg-blue-100 hover:text-blue-700 flex items-center gap-2 transition duration-300"
            onClick={handleViewFullInfo}
          >
            View full information
            <ExternalLink className="h-4 w-4" />
