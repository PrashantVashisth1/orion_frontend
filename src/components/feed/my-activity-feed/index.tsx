...
import { useNavigate } from "react-router-dom"
import ActivityPost, { type ActivityPostProps } from "../my-activity-post"
import { Users, Plus, Loader2 } from "lucide-react"
import { useActivityFeed } from "@/hooks/useActivityFeed"
...
export default function ActivityFeed() {
  ...
  // Transform posts to ensure type compatibility
  const activityPosts: ActivityPostProps[] = posts.map(post => ({
    id: post.id,
    text: post.text,
    images: post.images || [],
    ...
    comments: (post.comments || []).map(comment => ({
      ...
      user: {
        ...
        startup_profile: comment.user.startup_profile || null, // Ensure it's null instead of undefined
      }
    }))
  }))
