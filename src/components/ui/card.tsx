...
function Card({ className, children, ...props }: React.ComponentProps<"div"> & { children?: React.ReactNode }) {
  return (
    <div
      ...
      className={cn(
        // CLEANED UP STYLES:
        // Removed: before:bg-gradient-to-r, hover:shadow-blue-500, etc.
        // Kept: Basic card structure, border, and simple shadow.
        "bg-card text-card-foreground flex flex-col gap-6 rounded-xl border shadow-sm transition-shadow duration-300 ease-out hover:shadow-md",
        className
      )}
      {...props}
    >
...
function CardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card-header"
      className={cn(
        // Removed grid/container complexity for a cleaner standard header
        "flex flex-col space-y-1.5 px-6 py-6",
        className
      )}
      {...props}
    />
...
// Kept purely for compatibility if you use it elsewhere, simplified
function CardAction({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card-action"
